...
def filter_and_organize_by_weeks(src_root, dst_root, selected_weeds):
    """
    Filter dataset to include only selected weeds and organize by
...
def main():
    parser = argparse.ArgumentParser(description="Filter and organize dataset by weeks and selected weeds.")
    parser.add_argument('--src', type=str, default='../data', help='Source root directory')
    parser.add_argument('--dst', type=str, default='./data_filtered_by_weeks', help='Destination root directory')
    parser.add_argument('--weeds', '-w', nargs='+', required=True, help='List of selected weed names')

    args = parser.parse_args()

    print("Starting dataset filtering and week-based organization...")
    statistics = filter_and_organize_by_weeks(args.src, args.dst, args.weeds)
    ...
    print(f"\nFiltered dataset saved to: {args.dst}")
